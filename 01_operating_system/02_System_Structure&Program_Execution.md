...
>
>컴퓨터 시스템 구조, Mode bit, Timer, Device Controller, 입출력(I/O)의 수행, 동기식 입출력과 비동기식 입출력, 시스템콜(System Call), 인터럽트(Interrupt), 저장장치 계층 구조, 프로그램의 실행, 커널 주소 공간의 내용, 사용자 프로그램이 사용하는 함수
...
- 동기식 입출력 (Synchronous I/O)
  - IO 요청 후 입출력 작업이 완료된 후에야 제어가 사용자 프로그램에 넘어감 (I/O가 완료된 후에야 다음 명령을 실행할 수 있음)
  - 구현 방법 1
...
- 비동기식 입출력 (Asynchronous IO)
  - IO가 시작된 후 입출력 작업이 끝나기를 기다리지 않고 제어가 사용자 프로그램에 즉시 넘어감 (기다리지 않고 다음 명령을 실행할 수 있음)
